...
app.get('/', async (req,res) => {
    const credentialsPath = process.env.GMAIL_API_CREDENTIALS;
    const credentials = await fs.readFile(credentialsPath);
    const auth = await authenticate({
        keyfilePath: path.join(__dirname, credentialsPath),
        scopes: SCOPES,
    })
    ...
    async function loadCredentials() {
        const filePath = path.join(process.cwd(), credentialsPath);
